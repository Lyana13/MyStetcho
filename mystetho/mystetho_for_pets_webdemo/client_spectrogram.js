...
function setupWebSocket() {
	ws = new WebSocket("ws://mystetho-for-pets-cloud.herokuapp.com/spectrogram_ws");
	...
	ws.onmessage = function(e) {
	    ...
	    if (response['dataType'] == 'message') {
          ...
          if (response['data'] == 'fs loaded') {
              fs_loaded = true
          }
	    }
      else if (response['dataType'] == 'spectrogram_img') {

        displayPicture(response, "spectograma");

	    }
        else if (response['dataType'] == 'barplot_img') {
        // console.log('spectrogram received')
        // console.log(response['data'])

        displayPicture(response, "barplot");
...
function displayPicture(response, containerClass) {

    let base64=response['data'];
    let buffer=Uint8Array.from(atob(base64), c => c.charCodeAt(0));
    let blob=new Blob([buffer], { type: "image/gif" });
    let url=URL.createObjectURL(blob);
    let img=document.querySelector("." + containerClass + " img");
    img.src=url;
    img.classList.add("d");
}
